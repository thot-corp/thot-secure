...
Le texte ci-dessous n'affirme que des éléments du contrat d'interface ou du dépôt. Trois points exigent une vérification **avant** de soumettre, sinon la phrase doit être retirée :

1. **Fichier compose** — le contrat prévoit `deploy/` avec Dockerfile, compose, k8s, helm, terraform, ansible. Le fichier compose doit être **constaté sur disque** le jour J. Le `Makefile` du dépôt expose une cible `compose-up` : la citer comme source, pas comme preuve d'exécution.
...
3. **Gouvernance** — le `CONTRIBUTING.md` du dépôt renvoie à `GOVERNANCE.md`, cite une règle constitutionnelle (« pas de capacité offensive »), un DCO sans CLA, et l'exigence d'au moins une approbation de mainteneur avant fusion. Le fichier `GOVERNANCE.md` doit exister dans le dépôt public le jour J ; s'il n'y est pas, répondre « le document de gouvernance est référencé et publié avec la v0.1.0 » serait faux — il faut alors le dire franchement.
...
1. **Every action is reversible.** A playbook declares `reversible: true` and a `rollback:` block; `POST /api/v1/actions/{id}/rollback` must succeed while the rollback window is open. An action you can't undo doesn't go in a playbook.
...
**4. "Why SQLite?"**
Because the MVP is single-node and the alternative was shipping with no working default. `THOT_DB_URL` defaults to `sqlite:///./data/thotsecure.db`; PostgreSQL/TimescaleDB DDL is documented, not the default path. SQLite also makes the "install and look at it before you trust it" flow trivial — one file, no service to bring up. It is a real constraint: don't run multiple writers against it and expect Postgres behavior. If your deployment is multi-node, this is a documented migration, not something the code handles for you today.
