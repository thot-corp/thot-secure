...
|---|---|---|
| **PostgreSQL / TimescaleDB adapter** — first-class support alongside SQLite, selected through `THOT_DB_URL`. | ✅ **Done** (0.1.0) | The full test suite passes against PostgreSQL 16 and TimescaleDB 2.x; the retention job is implemented as a hypertable policy and verified by a test that ages events past `THOT_RETENTION_DAYS`; tenant isolation is re-verified on the new backend by the existing cross-tenant test. **Partially met, stated plainly**: the shared conformance suite runs against a real TimescaleDB service container in CI, but the maintainers have **not** operated it at production scale, and no large SQLite → PostgreSQL migration has been measured. |
| **Cloudflare WAF connector (real)** — `block_ip` / `unblock_ip` / rate-limit rules against the live API. | 🚧 **Code complete, live validation pending** | Connector passes an integration test against a recorded API fixture set **and** a manual validation run against a Cloudflare account documented in the pull request; rollback removes the exact rule it created, verified by reading back the rule list; API token scopes required are documented in `docs/`; behaviour when the API is unreachable is a recorded failure with no partial change, covered by a test. **The recorded-fixture half is done and covered by 30+ tests; the live-account run is not** — it needs a Cloudflare account and network access the maintainers do not have here, so the item stays open until someone performs it in a pull request. |
| **AWS WAF connector (real)** — IP sets and rate-based rules via the AWS API. | 🚧 **Code complete, live validation pending** | Same bar as the Cloudflare connector; IP-set update uses optimistic concurrency with a `LockToken` retry, rollback restores the previous IP set contents exactly, and a test asserts that a failed update leaves the set unchanged. **Met offline**: SigV4 is verified against the official AWS test vectors plus an independent reimplementation, the `LockToken` retry is retried exactly once and then reported, and a missing lock token prevents any write. Live-account validation remains, as does `rate_limit` (it would require editing a `RateBasedStatement` in a WebACL, whose reversibility is not guaranteeable — so it is deliberately not implemented). |
...
| **Anomaly detection (ML)** — statistical baselining on top of, never instead of, deterministic rules. | 🚧 Partly delivered in 0.1.0 | **Shipped in 0.1.0**: a statistical detector (EWMA + z-score; per-entity volume, source cardinality, first observation) with severity bands, guard-railed decisions and an explainable finding for every signal — see [`docs/detection/anomaly.md`](docs/detection/anomaly.md). What remains for this item is the **learning-based** layer: correlations between several entities (a source *and* an account, path distribution, time profile) and a measured false-positive rate on a reference dataset. The rest of the criterion is already met and remains binding: deviations produce a **finding with an explanation** (which signal, which baseline, which deviation), never an action on their own; the model runs offline with no telemetry leaving the deployment; deterministic rules remain able to suppress or override an anomaly finding. |
